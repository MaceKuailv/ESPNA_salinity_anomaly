...
save_path = filedb_lst[19+int(sys.argv[-1])]+'/ocean/wenrui_temp/particle_file/neutral2012/nc/'
...
vel = xr.open_mfdataset('/sciserver/filedb11-01/ocean/wenrui_temp/uall*', engine = 'zarr')
snap_ecco = xr.open_mfdataset('/sciserver/filedb0*-02/ocean/poseidon/daily_mean_ecco/zarr/snap*', engine = 'zarr')
vel['time_midp'] = snap_ecco['time_midp']
vel['utrans'] = vel['u']*vel.dyG*vel.drF
vel['vtrans'] = vel['v']*vel.dxG*vel.drF
vel['wtrans'] = vel['w']*vel.rA

ds = ecco_grid.swap_dims({'k': 'Z', 'k_l': 'Zl', 'k_p1':'Zp1','k_u':'Zu'}).rename({'tile':'face',
               'i':'X','i_g':'Xp1',
               'j':'Y','j_g':'Yp1',
...
# t_min = np.minimum(np.min(stops),p.t[0])
# t_max = np.maximum(np.max(stops),p.t[0])
# update_stops = p.ocedata.time_midp[np.logical_and(t_min<p.ocedata.time_midp,
#                                                      p.ocedata.time_midp<t_max)]
# print(p.ocedata.time_midp[0],p.ocedata.time_midp[-1])
# print(update_stops[0],update_stops[-1])
# raise
