...
seed = 2016
map_path = particle_path+'maps_animate/'
# output_path = particle_path+'output/' 
zarr_path = particle_path+'nc/'
vec0 = xr.open_zarr('/home/wjiang33/parallel_run/seaduck_post_process/sxsysz_mean')
oce = sd.OceData(vec0)

wrong = np.load('/home/wjiang33/parallel_run/seaduck_post_process/bolus_bug.npy')
...
vec_mean = xr.open_zarr('/home/wjiang33/parallel_run/seaduck_post_process/sxsysz_mean')
...
path_mid = 'ocean/wenrui_temp/particle_file/freshM/nc/'
which_node = list(range(9,19))
...
first_index = int(ds['index_of_time'].sel(time = dates[0][:10]).values)
to_isel = np.arange(first_index, first_index - len(dates),-1)
print('I want to print!!')
...
t1 = time.time()
for islice in range(len(dt)-1):
    the_slice = slice(dt[islice],dt[islice+1])
    print(
        islice, the_slice
    )
    for it,(dataset_date, particle_date) in enumerate(zip(to_isel[the_slice],dates[the_slice])):
        my = ds.isel(time = dataset_date)
        vec_i = vec.isel(time = dataset_date)
        # print(dataset_date, particle_date)
        ...
        for var in rhs_list:
            maps[var][islice] += cumu_map_zarr(neo, contr_dic, var, last).sum(axis = 0)
        maps['count'][islice] += cumu_map_array(neo, np.ones_like(tres_used)).sum(axis = 0)

maps_ds = xr.Dataset()
for var in rhs_list+['count']:
    maps_ds[var] = xr.DataArray(maps[var],dims = ('time','face','Y','X'))
