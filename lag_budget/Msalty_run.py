...
save_path = filedb_lst[19+int(sys.argv[-1])]+'/ocean/wenrui_temp/particle_file/saltyM/nc/'
...
XG = np.array(vel.XG[2])
YG = np.array(vel.YG[2])
Zl = np.array(vel.Zl)
np.random.seed(seed)
...
vols = np.array(vel.dxG[2,iy,ix])*np.array(vel.dyG[2,iy,ix])*np.array(vel.drF[iz])
Np = 1e6
